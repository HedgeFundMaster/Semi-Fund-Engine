...
# Import our custom modules for data handling and error management
from data_models import StandardColumns, DataFrameSchema, DataQuality, RequiredColumns, validate_dataframe_for_operation
from error_handler import handle_errors, SafeDataAccess, FallbackDisplays, ChartErrorHandler, ValidationHandler, ErrorMessages
            ...
            if len(selected_funds) >= 2:
                ...
                for ticker in selected_funds:
                    ...
                    comparison_data.append({
                        'Ticker': ticker,
                        StandardColumns.FUND: SafeDataAccess.safe_get_column(pd.DataFrame([fund_data]), StandardColumns.FUND, 'N/A').iloc[0],
    ...
    if not filtered_df.empty and len(filtered_df) > 1:
        ...
        with comparison_tabs[0]:
            ...
            if len(selected_comparison_funds) >= 2 and comparison_metrics:
                ...
                # Create comparison table with safe column access
                display_columns = ['Ticker']
                if StandardColumns.FUND in comparison_df.columns:
                    display_columns.append(StandardColumns.FUND)
                display_columns.extend([col for col in comparison_metrics if col in comparison_df.columns])
            ...
            if 'Total Return (%)' in filtered_df.columns and 'Volatility (%)' in filtered_df.columns:
                try:
                    ...
                    if len(plot_df) == 0:
                        ...
                        hover_data_cols = []
                        for col in [StandardColumns.FUND, 'Sharpe Ratio', 'Category']:
                            if col in plot_df.columns:
...
def create_portfolio_builder_tab(df_tiered):
    ...
    # Fund selection
    available_funds = SafeDataAccess.safe_get_column(valid_funds, StandardColumns.FUND, 'Unknown').tolist()
    ...
    # Safe filtering using standardized column names
    if StandardColumns.FUND in valid_funds.columns:
        portfolio_funds = valid_funds[valid_funds[StandardColumns.FUND].isin(selected_funds)].copy()
    else:
        st.error(f"Column '{StandardColumns.FUND}' not found in data")
        return
    ...
    with col1:
        ...
        try:
            ...
            fund_names = SafeDataAccess.safe_get_column(portfolio_funds, StandardColumns.FUND, 'Unknown').values
            ...
            allocation_df = pd.DataFrame({
                StandardColumns.FUND: fund_names,
                'Weight (%)': weights * 100,
...
def create_risk_analytics_tab(df_tiered):
    ...
    selected_fund = st.sidebar.selectbox(
        ...
        SafeDataAccess.safe_get_column(valid_funds, StandardColumns.FUND, 'Unknown').tolist(),
    ...
    with risk_tabs[0]:
        ...
        fig_risk_return, error_msg = safe_scatter_plot(
            ...
            size='Sharpe Ratio',
            color='Tier',
            hover_name=StandardColumns.FUND,
            color_discrete_map={
                'Top Tier': '#2E8B57',
                'High Tier': '#4682B4', 
                'Mid Tier': '#DAA520',
        ...
        if fig_risk_return is not None:
            ...
            display_cols = [StandardColumns.FUND, 'Total Return (%)', 'Volatility (%)', 'Sharpe Ratio', 'Tier']
        ...
        # Safe column selection for risk metrics
risk_cols = [StandardColumns.FUND, 'Total Return (%)', 'Volatility (%)', 'Sharpe Ratio', 'Tier']
available_risk_cols = [col for col in risk_cols if col in valid_funds.columns]
risk_metrics_df = valid_funds[available_risk_cols].copy()
    ...
    with risk_tabs[1]:
        ...
        # Safe fund selection
        if StandardColumns.FUND in valid_funds.columns:
            fund_matches = valid_funds[valid_funds[StandardColumns.FUND] == selected_fund]
            if not fund_matches.empty:
                selected_fund_data = fund_matches.iloc[0]
            else:
                st.error(f"Fund '{selected_fund}' not found")
                return
        else:
            st.error(f"Column '{StandardColumns.FUND}' not available")
            return
    ...
    with risk_tabs[2]:
        ...
        fig_corr = px.imshow(
            correlation_matrix,
            x=SafeDataAccess.safe_get_column(top_funds, StandardColumns.FUND, 'Unknown').str[:15],
            y=SafeDataAccess.safe_get_column(top_funds, StandardColumns.FUND, 'Unknown').str[:15],
            color_continuous_scale='RdBu',
            aspect='auto',
    ...
    with risk_tabs[3]:
        ...
        # Safe column selection for VaR analysis
var_cols = [StandardColumns.FUND, 'Total Return (%)', 'Volatility (%)']
available_var_cols = [col for col in var_cols if col in valid_funds.columns]
var_analysis_df = valid_funds[available_var_cols].copy()
        ...
        for conf_level in confidence_levels:
            fig_var.add_trace(go.Bar(
                ...
                x=SafeDataAccess.safe_get_column(var_analysis_df, StandardColumns.FUND, 'Unknown').str[:15],
        ...
        fig_var.update_layout(
            ...
            xaxis_title=StandardColumns.FUND,
            yaxis_title='VaR (%)',
            barmode='group',
            height=500,
            xaxis_tickangle=-45
